...
- **AWS CDK v2** (`aws-cdk-lib ^2.256.1`) with TypeScript
...
- **Amazon EventBridge** — event bus `OrcaBusMain`, source `orcabus.bclconvertinteropqc`
- **AWS SSM Parameter Store** — configuration under `/orcabus/workflows/bclconvert-interop-qc/`
- **AWS Schemas Registry** — JSON schema validation for draft events
- **Amazon ECS** — ECS task for resampling MultiQC parquet files

## Package Manager

**pnpm** (v11.1.3). Always use `pnpm`, never `npm` or `yarn`.
